...
import { Search, SlidersHorizontal, Eye, Tag } from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import Image from 'next/image';

export const revalidate = 0; // 停用快取以獲取最新資料
  ...
  return (
    <div className="marble-bg min-h-screen">
      <div className="mx-auto max-w-7xl px-6 lg:px-8 py-12">
        ...
        {/* Filter and Search Bar (GET Form) */}
        <form method="GET" action="/gallery" className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-card/60 backdrop-blur-sm p-4 rounded-xl border border-border/80 mb-10 shadow-sm">
          {/* Search */}
          ...
          <div>
            <select
              name="type"
              defaultValue={type}
              className="block w-full rounded-lg border border-border bg-background py-2.5 px-3 text-sm text-foreground focus:outline-none focus:border-primary transition-colors"
            >
          ...
          <div className="flex items-center gap-4 justify-between">
            ...
            <button
              type="submit"
              className="rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 px-5 py-2.5 text-sm font-medium shadow-sm transition-all flex items-center gap-1.5"
            >
                ...
                <div className="flex flex-1 flex-col p-5 bg-card border-t border-border/30">
                  ...
                  <div className="mt-5 pt-3 border-t border-border/60 flex items-end justify-between">
                    <div>
                      ...
                      {artwork.is_rentable && (
                        <p className="text-xs text-indigo-900 dark:text-indigo-200 font-medium mt-0.5 font-mono">
